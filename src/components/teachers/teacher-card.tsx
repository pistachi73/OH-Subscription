...
export const TeacherCard = ({ teacher }: TeacherCardProps) => {
  return (
    <m.article
      initial="initial"
      whileHover="hover"
      className="relative h-full w-full aspect-[6/7]  bg-muted rounded-xl shadow-md  flex-row gap-0 overflow-hidden"
    >
      <Link
      ...
      >
        ...
        <section
          className={cn(
            "absolute bottom-0 left-0 z-20 p-3 flex flex-col w-full h-full justify-end",
            "before:absolute before:-z-10 before:left-0 before:bottom-0 before:h-full before:w-full before:bg-gradient-to-t from-muted-background/70 to-background/0 to-50% ",
          )}
        >
